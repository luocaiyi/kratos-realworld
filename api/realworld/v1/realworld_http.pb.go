...
const OperationRealWorldAuthentication = "/realworld.v1.RealWorld/Authentication"
...
func RegisterRealWorldHTTPServer(s *http.Server, srv RealWorldHTTPServer) {
	r := s.Route("/")
	r.POST("/api/users/login", _RealWorld_Authentication0_HTTP_Handler(srv))
...
func _RealWorld_Authentication0_HTTP_Handler(srv RealWorldHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in AuthenticationRequest
		if err := ctx.Bind(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationRealWorldAuthentication)
...
type RealWorldHTTPClient interface {
	...
	Authentication(ctx context.Context, req *AuthenticationRequest, opts ...http.CallOption) (rsp *UserReply, err error)
...
func (c *RealWorldHTTPClientImpl) Authentication(ctx context.Context, in *AuthenticationRequest, opts ...http.CallOption) (*UserReply, error) {
	var out UserReply
	pattern := "/api/users/login"
	path := binding.EncodeURL(pattern, in, false)
	opts = append(opts, http.Operation(OperationRealWorldAuthentication))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "POST", path, in, &out, opts...)
	if err != nil {
		return nil, err
